...
Estimated marginal effects are usually presented in an aggregated form. For each regressor, we usually report two (sometimes three) statistics:
 \begin{itemize}
    \item Summary indicator for direct effects is calculated as the average of all diagonal elements (Jacobian for a given regressor $\bm{x}_k$). 
    \medskip
...
\begin{itemize}
    ...
    \smallskip
    \item Generally, $\bm{\beta}$ parameters do not constitute marginal effects -- direct/indirect effects have to be calculated (in principle, using the same approach as for CS data).
    \smallskip
    \item With the ``random effects'' (RE) models, we assume that unobserved individual effects $\bm{\mu}$ are not correlated to other regressors of the model. 
    \smallskip
...
$
\begin{itemize}
    \item https://www.jstatsoft.org/article/view/v047i01
\end{itemize}
\end{frame}
%---------------------------------------------------------------------
