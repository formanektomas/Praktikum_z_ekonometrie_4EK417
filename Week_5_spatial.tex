...
\begin{figure}
	...
	\caption{The relationship between different spatial dependence models for cross-sectional data (source: Elhorst, 2014). Note: this illustration uses a slightly modified notation of spatial parameters.}
\end{figure}
\end{frame}
%---------------------------------------------------------------------
...
\frac{\partial E(\bm{y})}{\partial E(\bm{x}_k)}
	...
	\end{pmatrix}\,. 
\end{equation*}
\item Recall that $w_{ij}$ denotes element of the $\bm{W}$ matrix and $w_{ij} > 0$ if two spatial units $i$ and $j$ are neighbors (zero otherwise). $\beta_k$ and $\theta_k$ are parameters of model GNS/SDM, corresponding to the $k$-th regressor. The RHS is a $N\!\times\!N$ matrix.
\end{itemize}	
\end{frame}
%---------------------------------------------------------------------
...
\item Statistickou významnost přímých efektů můžeme testovat viz \href{http://www.springer.com/cda/content/document/cda_downloaddocument/9783642403392-c2.pdf}{Elhorst (2014)}.	
\end{itemize}
\end{frame}
%---------------------------------------------------------------------
