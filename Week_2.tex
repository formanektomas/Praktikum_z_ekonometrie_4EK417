...
\begin{frame}
\frametitle{4EK417 Block 1 topics: Outline} % Table of contents slide, comment this block out to remove it
...
\end{itemize}
\bigskip
The following discussion only focuses on the PCA/PCR approach.
\end{frame}
%------------------------------------------------
...
\bm{z}_m = \sum_{j=1}^p \phi_{mj} \bm{x}_j,
\end{equation}
...
\begin{frame}{Partial least squares (PLS)}
\begin{itemize}
\item Much like with PCR, in PLS we also search for convenient (aggregating) linear combinations of regressors in matrix $\bm{X}$.\\
\medskip
...
%---------------------------------------------------------------------
\begin{frame}{Partial least squares (PLS)}
Fist component for PLS:

$$ \bm{z}_1^{\textnormal{PLS}} = \sum_{j=1}^p \psi_{j1}\bm{x}_j$$
\medskip
where coefficients $\psi_{j1}$ are calculated in two steps:
\begin{enumerate}
\item Use OLS to estimate slope-coefficients of the ``simple'' \\linear regressions $\bm{y} \leftarrow \bm{x}_j$.\\
\medskip
\item Standardize the coefficients so that $\sum_{j=1}^p \psi_{j1}^2=1$.
\end{enumerate}
\bigskip
...
PLS -- detailed technical description and estimation algorithm:
\begin{itemize}
