...
$$
\begin{itemize}
\item Same way as OLS, ridge regression seeks coefficient estimates that fit the data well, by making the RSS small.
\smallskip
...
\smallskip
\item The second term, $\, \lambda \sum_{j=1}^p  \hat{\beta}_j^2 \,$ is called a shrinkage penalty.\\It is small when $\hat{\beta}_1, \dots, \hat{\beta}_p$ are close to zero and/or $\lambda$ is small. \\High $\lambda$ has the effect of shrinking $\hat{\beta}_j^2$ towards zero.
\smallskip
