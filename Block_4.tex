...
\begin{itemize}
    ...
    \bigskip
    \item \textbf{Random effects:} typically, grouping factors that we are trying to control for. \\ \medskip RE are always associated with categorical variables (\texttt{R} cannot treat continuous variables as random effects). \\ \medskip 
...
\medskip
\item The observation set $\{y_{ti},\bm{x}_{ti}\}$ denotes some $i$th individual  observed at a given time period $t$. The number of observations in time may differ among CS units and observations may occur at different time points. \\ \medskip 
