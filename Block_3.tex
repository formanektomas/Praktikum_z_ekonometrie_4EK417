...
Regardless of the algorithm used neighbor vs non-neighbor categorization, spatial structure may be formalized using a connectivity matrix $\bm{C}$:
$$
~~\bm{C} = \begin{bmatrix}
0 & 1 & 1 & 1 \\
1 & 0 & 1 & 0 \\
...
%---------------------------------------------------------------------
\begin{frame}{Spatial dependency}
Model $\bm{y} = \lambda \bm{W\!y} + \bm{\varepsilon}$ example: \\(simulated data for different $\lambda$ values and \textit{iid} residuals $\bm{\varepsilon}$)
\vspace{-0.3cm}
\begin{figure}[htbp]
  \centering
