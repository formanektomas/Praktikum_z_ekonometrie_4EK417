...
\medskip GAMs extend LRM by allowing non-linear functions in each variable, while maintaining additivity (and ceteris paribus interpretation!).\\ \medskip
Multiple-regressor LRM:
$$ y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \cdots + \beta_p x_{ip} + u_i~~~~~~~~~~~~~~~~~$$
can be generalized to
$$ y_i = \beta_0 + \beta_1 \, f_1(x_{i1}) + \beta_2 \, f_2(x_{i2}) + \cdots + \beta_p \, f_p(x_{ip}) + u_i$$ 
where $f_j(x_{ij})$ represents any convenient function of $x$: piecewise constant, linear, polynomial, spline, etc.
\end{frame}
%---------------------------------------------
\begin{frame}{GAM example}
$\texttt{wage} = \beta_0 + \beta_1 \, f_1(\texttt{year}) + \beta_2 \, f_2(\texttt{age}) + \beta_3 \, f_3(\texttt{education}) + u_i$\\
\bigskip
...
Ceteris paribus, \texttt{wage} tends to be highest for intermediate values of  \texttt{age} \\and lowest for the very young and very old workers.
\end{frame}
%---------------------------------------------
...
\begin{itemize}
    ...
    \smallskip
    \item We do not use an expansion of basis functions for estimation \\(say, all constituent parts of a natural spline $f_j(x_{j})$ function). \\If all such elements were used in matrix of regressors $\bm{X}$, the model could be estimated by least squares.
    \smallskip
...
\begin{itemize}
    ...
    \right) = \beta_0 + \beta_1 \, f_1(\texttt{year}) + \beta_2 \, f_2(\texttt{age}) + \beta_3 \, f_3(\texttt{education}) + u_i$$
...
\begin{frame}{Generalized Additive Models (GAM)}
\begin{itemize}
    \item[\ding{51}] As GAMs are additive, they provide ceteris paribus interpretation. 
    \bigskip
    ...
    \bigskip
    \item[\ding{55}] Additive nature of GAMs may lead to missing important interactions. \\ \smallskip However, low-dimensional interactions can be included manually; e.g. by adding some $f_{jm}(X_j,X_m)$ term into the model.
\end{itemize}
\end{frame}
%---------------------------------------------
\begin{frame}{Final note}
\centering
For a detailed technical discussion of the above topics,\\ \medskip you may consult e.g.: \textcolor{blue}{\underline{\href{https://web.stanford.edu/~hastie/ElemStatLearn/}{The Elements of Statistical Learning}}}
\end{frame}
%---------------------------------------------
