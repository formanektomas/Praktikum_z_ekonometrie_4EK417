...
\begin{figure}
  \centering
  \includegraphics[trim=0cm 0cm 10.5cm 9.5cm, clip=true, width=0.4\textwidth]{IMG/ISLR73.pdf}
\end{figure}
\end{frame}
%---------------------------------------------
\begin{frame}{Regression splines (cubic spline)}
...
%---------------------------------------------
\begin{frame}{Regression splines (natural spline)}
Example: For $K=4$ and using the truncated power bases of cubic splines, we can write:
$$y_i = \beta_0 \, N_1 (x_i)  + \beta_1 \, N_2 (x_i) + \beta_2 \, N_3 (x_i) + \beta_3 \, N_4 (x_i) + u_i  $$\\
\vspace{-0.2cm}
where:\\
\smallskip
\qquad $N_1 (x_i) = 1$\, \qquad ($\beta_0$ is the intercept),\\
\smallskip
\qquad $N_2 (x_i) = x_i$\,,\\
\smallskip
...
\medskip
Note that $N_{k+2}=d_k(X)-d_{K-1}(X)$ is the general notation \\for $N_3$ and $N_4$ and we only use knots up to $K-2$ (i.e. $\xi_1, \xi_2$).\\ All basis functions $N$ have zero $2^{nd}$ and $3^{rd}$ derivatives \\for $X < \xi_1$ and for $X \geq \xi_K$.
\end{frame}
%---------------------------------------------
...
\begin{itemize}
    ...
    \medskip    
    \begin{itemize}
        \item Place more knots in places where we assume the function \\may vary most rapidly.
        \smallskip
        \item Place fewer knots where the function seems more stable.
        \smallskip
        \item Ad-hoc and potentially missleading approach.
    \end{itemize}
\end{itemize}
\end{frame}
%---------------------------------------------
