...
\bigskip
where $f_1$ and $f_2$ are smoothing splines and $f_3$ is a step function (\texttt{education} is a qualitative variable with four levels).
\vspace{-0.2cm}
\begin{figure}
  \centering
...
\begin{itemize}
    ...
    \smallskip
    \item Instead, we fit all $p$ functions ``simultaneously'', using a scatterplot smoother (e.g. smoothing spline, natural spline, polynomial function, etc.) , using a \textbf{backfitting algorithm}.
    \smallskip
