...
$$y_i = \beta_1 \, N_1 (x_i)  + \beta_2 \, N_2 (x_i) + \beta_3 \, N_{2+k} (x_i) + \cdots + \beta_{K} \, N_{2+K-2} (x_i) + u_i  $$\\
\vspace{-0.2cm}
~\,where:\\
\smallskip
\qquad $N_1 (x_i) = 1$\,,\\
...
\qquad $N_2 (x_i) = x_i$\,,\\
\smallskip
\qquad $N_{2+k} (x_i) = d_k(X)-d_{K-1}(X)\,$,\\
\qquad $\cdots$\\
\qquad $N_{K} (x_i) = d_{K-2}(X)-d_{K-1}(X)\,$,\qquad (note that $K=2+K-2$),\\
\bigskip
...
\begin{itemize}
    \item Smoothing splines -- alternative to cubic \& natural splines.\\ \smallskip
    Essentially, a penalized natural cubic spline with $N$ knots \\(i.e. knot at each observation of the regressor).
    \bigskip
